...
My shell installation procedure is described https://github.com/Jubijub/arch-config/wiki/5.Post-installation[here].

* https://fishshell.com[fish] : my main shell, for its completions and built-in suggestions. Configured with https://github.com/jorgebucaran/fisher[Fisher] plugins and https://starship.rs[Starship].
* https://www.zsh.org[zsh] : kept only as a POSIX-ish fallback for the rare cases where fish does not fit. Deliberately plugin-free : no oh-my-zsh, no package manager, no theme engine.
